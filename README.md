...
- **[Traversable-Wormhole-Inspired Quantum Teleportation](#21-traversable-wormhole-inspired-quantum-teleportation-syk-model)** — real Gao-Jafferis-Wall protocol on a binary sparse SYK model: sign-dependent teleportation signal requires the injected message (control gives I=0 exactly), a real 870-point 2D grid search over (t0, mu) finds a maximum at t0=0.65, mu=15.0, and iterating that re-scan (alternating t1 scans and (t0, mu) grids) converges after 3 rounds to a genuine joint fixed point at t0=0.70, mu=17.0, t1=0.36, +44.6% over the original headline value (delta=+0.01688 vs. +0.01167) — **but an honest generality check across 5 more independent SYK instances shows this converged point does NOT generalize**: it's a property of the one instance tested (seed=61), not the protocol.
