...
- **[Gate Blocking Redesign: Promotion-Ready](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_gate_blocking_redesign_v2/)** -- closes three real gaps found before promoting gate blocking into `dense_evolution` (row encoding, untested SWAP-chain/CCX interactions, bookkeeping granularity -- decided as an opt-in `fuse_gates=True` flag); re-confirms 2.77x on the complete, real-`_compile_mps_ops`-based pipeline.
- **[Gate Blocking: Closing the GPU Gap](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_gate_blocking_experiment/)** -- fusing consecutive same-qubit gates into one matrix on the host (exact, motivated by two papers found via quantumrag) halves the MPS scan step count and, measured through one consistent methodology end to end, gives a real 2.87x GPU speedup over the pre-optimization baseline -- clears the 2x target the earlier bucketed-SVD GPU claim missed.
- **[Bucketed-SVD MPS: GPU Timing Correction](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_bucketed_svd_gpu_timing_followup/)** -- the originally claimed 2.74x GPU speedup was measured on a standalone reimplementation, never the real promoted `run_circuit_jit` API; root-caused to a benchmark methodology bug (a fresh instance never reuses a compiled kernel), corrected to a real ~1.41x for bucketing alone.
- **[Bucketed-SVD MPS Optimization](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_bucketed_svd_optimization/)** -- `run_circuit_jit`'s fixed-size SVD (always padded to `max_bond`) replaced with a runtime-dispatched, provably-bounded bucket size; verified correct across ~92 single-gate configurations plus a real multi-gate circuit check. CPU speedup (68.80x-73.96x) verified via the real API and stands; GPU number superseded, see the two entries above.
- **[native_hf: d-Shell Support](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/native_hf_d_shell_experiment/)** -- runs a full RHF/6-31G* SCF on neon (a real Cartesian d shell, not just s/p) end to end, agreeing with an independent PySCF calculation to ~1e-12 Hartree once a classic spherical-vs-Cartesian-d convention mismatch is accounted for.
...
`six_dof_pbc_cbf_controller.py` extends Experiment 63's controller from position-only to the link's full 6-DoF pose, using the spatial Jacobian and Lee, Leok & McClamroch (2010)'s SO(3) attitude error instead of Kurtz et al.'s own gimbal-lock-prone RPY error; verified by an exact gravity-compensation check at zero error (machine precision), a real closed-loop run converging a 10cm/30-degree offset to near-zero (position 1e-10 m, orientation 3e-8) over 1500 steps, and validated on the same 3-robot bar as Experiment 63 -- which surfaced and fixed a real second-level OSQP infeasibility unique to the 6-DoF manipulability measure.
