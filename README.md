...
- **[Traversable-Wormhole-Inspired Quantum Teleportation](#21-traversable-wormhole-inspired-quantum-teleportation-syk-model)** — real Gao-Jafferis-Wall protocol on a binary sparse SYK model: sign-dependent teleportation signal requires the injected message (control gives I=0 exactly), a real 870-point 2D grid search over (t0, mu) finds a maximum at t0=0.65, mu=15.0 (computed in 47.6s instead of an estimated ~2 hours after precomputing the instance's Hamiltonians once instead of per grid point, ~165x speedup) — and a follow-up t1 re-scan at that point finds the true peak at t1=0.42, not the 0.60 the grid had held fixed, ~30% higher signal (delta=+0.01518 vs. +0.01167).
