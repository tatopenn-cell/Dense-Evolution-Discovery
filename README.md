...
One claim was different: that the **order** of applying two distinct noise channels leaves a measurable fingerprint — the archive's own "Attrattori Crono-Topologici" idea. `scripts/channel_order_noncommutativity.py` tests this directly on the Regola 16 circuit (`GHZ(3q) -> X(Q0) -> Z(Q1) -> X(Q2) -> CNOT(Q0,Q2)`): dephasing-then-amplitude-damping vs. amplitude-damping-then-dephasing, same intensities (p=0.3 each), 8192 Monte Carlo trajectories per order.

**Result: real, not noise.** State `|000⟩` lands at 17.4% under dephasing→AD vs. 13.3% under AD→dephasing. Jensen-Shannon divergence between the two output distributions is 0.00174 — a permutation test (300 reshuffles of the trajectory pool) gives a null distribution topping out at 0.00054, about a third of the observed value (p=0.0033). Quantum channels genuinely don't commute in general; this is a real, honestly-confirmed instance of it, not an artifact — the one claim from this whole archive that survived contact with a fair, statistically rigorous test.
