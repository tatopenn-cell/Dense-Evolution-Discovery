...
- **[Leaky-Switch Differentiable Healing](#28-leaky-switch-differentiable-healing-real-gradients-poor-healing-quality)** — a Colab-proposed JAX/XLA rewrite of the healing filter (sigmoid soft-trigger instead of a hard branch, meant to keep gradients flowing through a training loop) really is differentiable: 30/30 seeds give finite, nonzero gradients, though the "leaky" floor meant to prevent dead gradients made no measurable difference in any of them. But healing *quality* is clearly worse than what is already shipped — cosine alignment with the ground truth of ~0.06 (barely above chance) versus ~0.77-0.85 for Experiment 27's adaptive trigger, on every corruption type tested.
...
### 26. Stratonovich-Projection Vector Healing: a Real but Partial Win, Diluted by an Over-Firing Trigger

**What's being tested:** a Google Colab session read Hu & Sverak's paper on stochastically perturbed Euler-Arnold flows (arXiv:1510.05279) and proposed replacing `ia_utils.vector_healing`'s median-fallback correction with a "Stratonovich projection" (local-mean baseline plus a damped drift term along the recent finite-difference velocity direction) — a single-seed demo (seed=42, one spike corruption) reported a dramatic win, cosine phase alignment going from -0.16 to +0.98. **What we find:** tested properly — 4 corruption types (single spike, NaN string, scattered outliers, spike+NaN combined) x 40 seeds each, same production Phi-Trigger (`dense_evolution.mitigation.healing.evaluate_phi_trigger`) for both correction methods, paired Wilcoxon significance tests — the picture is real but much smaller and scenario-dependent than the demo suggested. Restricted to exactly the corrupted indices (the Colab's own comparison design, bypassing the trigger): Stratonovich correction beats the median fallback on spike-type corruption — single spike L2 error 7.38→4.12 (-44%, 100% of seeds, p<10^-11), scattered outliers 21.45→17.75 (98% of seeds), combined 19.47→17.09 (100% of seeds) — but *loses* on pure NaN-gap corruption, 18.36→19.37 (0% of seeds, p<10^-10). Even in the winning cases, absolute cosine alignment stays deeply negative (-0.72 to -0.04) — nowhere near the demo's +0.98, which was a favorable artifact of that one seed, not the general behavior. **A separate finding:** the shipped production Phi-Trigger fires on 85-90% of *every* trajectory tested — corrupted or not — so in the real pipeline (not the forced/oracle-index test above), the correction-step choice barely moves total L2 error and the "winner" flips depending on corruption type, because correction is applied almost everywhere rather than selectively at real anomalies.
...
### 28. Leaky-Switch Differentiable Healing: Real Gradients, Poor Healing Quality

**What's being tested:** the same Colab session behind Experiments 26-27 also proposed a JAX/XLA rewrite of the healing filter using `jax.lax.scan` and a sigmoid "soft trigger" (blending the raw and healed values continuously) instead of a hard if/else branch, plus a small "leaky" floor (`trigger_activation * (1 - 1e-4)`) specifically meant to prevent the sigmoid from fully saturating and killing the gradient -- intended for use cases where healing sits inside a differentiable training loop (e.g. a VQE loss). The Colab's own validation was a single 4x8 toy gradient check on one seed, and never checked healing quality at all, only that a gradient existed. **What we find, on two separate questions:**

**1. Differentiability -- real, as claimed.** 30/30 seeds give finite, non-zero `jax.grad` on a `sum(healed**2)` loss (gradient norm 48.7-71.0). But the "leaky" 1e-4 floor turned out to be unnecessary insurance for every case tested: an ablation removing it entirely (`epsilon=0`) *also* gave 30/30 finite non-zero gradients, with gradient norms within 0.1% of the leaky version -- the plain `jax.nn.sigmoid` never saturated hard enough in these trials to need the floor.

**2. Healing quality -- clearly worse than what's already shipped.** Compared against Experiment 27's `trigger_mode='adaptive'` on the same 4 corruption scenarios x 40 seeds, this design's cosine alignment with the ground truth sits at ~0.055-0.058 (barely above chance) versus ~0.77-0.85 for the adaptive trigger -- a 0-10% win rate for the leaky design, every comparison significant at p<0.0001. L2 error is also suspiciously near-constant (~56.7-56.8) across every corruption type, the signature of a filter that smooths indiscriminately rather than correcting the actual anomalies. Three concrete structural reasons stand out: a *fixed* 0.25 sigmoid threshold (not the MAD-adaptive one from Experiment 27), zero-imputation of NaN/Inf instead of column-mean imputation, and `jax.lax.scan` cascading each correction into the next window's statistics (the shipped `enhanced_dense_healing_hybrid` deliberately does not cascade).

**On the arXiv:1510.05279 citation:** as with Experiments 26's Stratonovich correction, re-reading the actual Hu & Sverak paper turned up nothing that could improve this specific design. Two concrete reasons: the paper's math requires a genuine finite-dimensional Lie algebra with known structure constants to define its Euler-Arnold bracket q(z,z) -- arbitrary hidden-state vectors have no such structure, so there is no principled way to make the heuristic `q_force = ipg_vector * norm_A` more faithful to it. More directly, the paper's central concern -- Stratonovich vs. Ito integration to keep a stochastic process on a constraint manifold -- doesn't even apply here, because this design has no stochastic noise term at all (`healed = state_A + drift_step * 0.5` is fully deterministic).

**Verdict:** the differentiability engineering is sound and could be useful as a starting point if a real need for gradient-through-healing ever arises, but this exact design should not replace anything currently shipped -- it would need the same MAD-adaptive threshold and NaN-handling fixes from Experiment 27, rewritten differentiably, before it would be competitive.

Produced by `scripts/leaky_differentiable_healing.py` → `data/leaky_differentiable_healing.csv`.
