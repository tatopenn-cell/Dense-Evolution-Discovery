...
- **[MPS GPU Optimization: Bucketed SVD + Gate Blocking](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_gpu_optimization/)** -- `run_circuit_jit`'s SVD size and gate scheduling both optimized (opt-in `fuse_gates=True` flag), with three real bugs found and fixed only by testing the literal public API on real GPU hardware; final validated result: **2.16x** faster on Kaggle T4, CPU speedup 68.80x-73.96x.
