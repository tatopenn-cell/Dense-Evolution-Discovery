...
Real reproduction of the Gao-Jafferis-Wall traversable-wormhole teleportation protocol on a chaotic binary sparse N=8 Sachdev-Ye-Kitaev (SYK) model, following **arXiv:2604.10090** — built on `dashboard_core.wormhole` (main library, `dense-evolution>=8.1.49`). An earlier, discarded circuit used the right vocabulary but wasn't real physics (a single-qubit-register readout structurally can't show sign-dependent behavior, per the no-signaling theorem, verified directly). The real protocol needs two coupled chaotic systems, a message injected via a separate reference-qubit pair, a real bilinear L-R coupling, and a mutual-information readout — all implemented and unit-tested in the main repo. Instance selection matters: `select_good_instance` reproduces the paper's own commuting/anticommuting term-pair-count criterion (seed 61 matches the paper's 34/11 ratio exactly, used throughout).
...
| realistic noise robustness (Trotter, depolarizing) | p where signal crosses zero | ~~between p=0.01 and p=0.02~~ **re-verified 2026-08-13: does NOT cross zero up to p=0.20** (that claim predated the v8.1.57 noise-model fix, see correction below) |
...
| **term-order x noise interaction check (n=6 -> n=50, verified 4x)** | noisy_order_sensitivity vs. sign | r=+0.811 (p=0.050) -> +0.587 -> +0.396 -> r=+0.340 (p=0.0158) at n=50, significant at every step — **re-verified 2026-08-13 against the corrected noise model: r=+0.4013 (p=0.0039), holds and strengthens** (see correction below) |
...
The signal requires enough pre-coupling scrambling before it appears and vanishes exactly (not just approximately) when the message injection is removed, confirming the sign-dependent asymmetry genuinely comes from the teleported message. A joint (t0, mu, t1) coordinate-ascent search converges to a fixed point **+44.6% over the original 1D-scan value** — but that converged point does **not generalize** across 5 more independently-selected SYK instances (a first honest negative result). The signal also decays with noise on a real Trotterized circuit, though **not as fast as first reported** (see correction below). Re-evaluating the paper's own "generic feature of the ensemble" claim at n=100 found **49/100 (49%) wrong-signed** — later corrected to **41/100 (41%)** after catching a mislabeled default parameter (the paper's real hardware working point is `t0=1.8`, not `t0=0.3`) — either way, close to a coin flip, not "generic." Eight follow-up candidate explanations were tested for what predicts the sign (mode-usage imbalance, spectral level-spacing, size winding, message-mode participation, operator growth rate, coupling topology, system-size scaling, term-order non-commutativity) — seven come up empty; only the **interaction** between term-order and physical noise shows a real, sample-size-stable correlation (r=+0.340, p=0.0158 at n=50). Full write-up, all 18 sub-experiments and the size-winding implementation bug caught and fixed along the way: [`docs/wormhole_syk_teleportation.md`](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/wormhole_syk_teleportation/). Produced by `scripts/wormhole_syk_teleportation.py` → `data/wormhole_*.csv`.

**Correction (2026-08-13): the noise-robustness claim above was checked against a buggy noise model.** Experiment 9 (the noise scan) ran 2026-08-07, four days before `dense-evolution` v8.1.57 (2026-08-11) fixed a real bug in `NoiseModel.apply_to_sv`'s depolarizing channel — it used to draw 2^(n-1) independent fire/no-fire decisions per qubit per shot instead of one, over-decohering entangled states by up to ~2.5x nominal p. Re-run against v8.1.60 (`scripts/wormhole_noise_scan_reverified.py`, JAX-`vmap`-batched via `NoiseSpec` for n=500 trials/point instead of the original n=6 — verified first against the eager path bit-for-bit on the noiseless case): the signal **does not cross zero** anywhere in the originally-tested range (0.0 to 0.05), and stays positive out to p=0.20 (delta=+0.0035, ~4 SEM above zero) before continuing to decay. The original n=6 budget, sized for the old, much-more-aggressive buggy channel, also turned out to be too small for the corrected (weaker, single-shot) channel at low p — several points showed exactly zero trial-to-trial variance simply because none of 6 trials happened to draw any error at all (real ~16% chance at p=0.01, not a bug). **Experiments 17 and 19** (the term-order x noise interaction check and its noise-level scan) **also predated the v8.1.57 fix — now re-verified** (`scripts/wormhole_term_order_noise_reverified.py`, same `NoiseSpec`-wrapped `jax.vmap` approach, verified bit-for-bit against `_run_trotter_ordered`'s eager reference on the noiseless case first). The flagship Experiment 17 result **holds and strengthens**: r=+0.340 (p=0.0158) → **r=+0.4013 (p=0.0039)** at n=50, noise_p=0.01. Experiment 19's noise-level scan (n=20, same seed set, common random numbers reused across noise_p so the three points aren't confounded by a different random draw each time) tells a different story than before, though: the correlation's *trend* has reversed. Originally it strengthened with **increasing** noise (p=0.374 → 0.0065 → **0.0034** at p=0.005/0.01/0.02); re-verified, it's now strongest at the **lowest** tested noise and vanishes at the highest:
...
The single-point flagship claim (noise_p=0.01) survives and strengthens; the multi-point trend claim (correlation gets stronger as noise increases) does not — it was itself an artifact of the same buggy, over-aggressive noise channel Experiment 9's claim was checked against. Produced by `scripts/wormhole_noise_scan_reverified.py` → `data/wormhole_trotter_noise_scan_reverified_v8160.csv` and `scripts/wormhole_term_order_noise_reverified.py` → `data/wormhole_term_order_noise_interaction_reverified_v8160.csv`, `data/wormhole_noise_level_scan_reverified_v8160.csv`.

[![Re-verified noise robustness vs. corrected noise model (v8.1.60): does not cross zero up to p=0.20](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.23.0/wormhole_trotter_noise_scan_reverified_v8160.png)](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.23.0/wormhole_trotter_noise_scan_reverified_v8160.png)

[![Re-verified Experiment 17: term-order x noise interaction, corrected noise model, n=50](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.24.0/wormhole_term_order_noise_interaction_reverified_v8160.png)](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.24.0/wormhole_term_order_noise_interaction_reverified_v8160.png)

[![Re-verified Experiment 19: noise-level scan, corrected noise model, trend reversed](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.24.0/wormhole_noise_level_scan_reverified_v8160.png)](https://github.com/tatopenn-cell/Dense-Evolution-Discovery/releases/download/v2.24.0/wormhole_noise_level_scan_reverified_v8160.png)
