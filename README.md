...
- **[Bucketed-SVD MPS Optimization](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/mps_bucketed_svd_optimization/)** -- `run_circuit_jit`'s fixed-size SVD (always padded to `max_bond`) replaced with a runtime-dispatched, provably-bounded bucket size; verified correct across ~92 single-gate configurations plus a real multi-gate circuit check, 68.80x-73.96x faster on CPU and 2.74x on GPU (both clear the 2x target), with the GPU-vs-CPU speedup gap traced directly to per-step kernel-launch overhead, not a broken dispatch (confirmed via a forced-branch HLO check). Promoted into Dense-Evolution.
