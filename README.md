...
- **[native_hf: d-Shell Support](https://tatopenn-cell.github.io/Dense-Evolution-Discovery/native_hf_d_shell_experiment/)** -- runs a full RHF/6-31G* SCF on neon (a real Cartesian d shell, not just s/p) end to end, cross-checking the new per-orbital normalization against independent scipy quadrature and the result against the neon atom's known numerical Hartree-Fock limit.
