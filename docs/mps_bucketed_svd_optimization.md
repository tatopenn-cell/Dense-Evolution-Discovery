...
**Correction (2026-09-10):** the GPU number below (2.74x) was measured on a standalone reimplementation of the bucketed dispatch, not on the real `MPSSimulator.run_circuit_jit` API this was promoted into (Dense-Evolution PR #226). When `run_circuit_jit` itself was benchmarked on GPU afterward -- with byte-identical kernel source confirmed against the installed package -- it measured slower (8.6-8.9s warm) than the pre-optimization baseline (6.60s), not faster. The CPU number (68.80x-73.96x) *was* measured via the real API and stands. The GPU claim below is **unverified against production code and currently contradicted by the one real-API measurement taken** -- treat it as withdrawn pending a clean re-test. See [mps_bucketed_svd_gpu_timing_followup.md](mps_bucketed_svd_gpu_timing_followup.md) for the investigation.
