...
That single direction is proposed here to be a topological entanglement connecting the whole universe. Real physics does place entanglement at the center of two things this framework leans on: a real experimental test (Salart et al. 2008 [22]) put a lower bound of roughly 10,000&times; the speed of light on any hypothetical hidden influence coordinating entangled particles across an 18&nbsp;km baseline &mdash; though the standard reading of that result, worth stating plainly, is that no usable signal or information travels at that bound; the correlation is real, but nothing is communicated faster than light by it. Separately, and independently of that experiment, Maldacena & Susskind's ER=EPR conjecture (2013 [23]) proposes that any maximally-entangled pair of particles is connected by a wormhole (an Einstein-Rosen bridge, at the Planck scale, non-traversable) &mdash; that entanglement and wormhole geometry may be the same phenomenon described two ways. Reading the universe's single topological direction as this kind of entanglement, connecting everything within it, is this framework's own extension of that idea, not a claim already established by either cited result.
...
None of the four applications currently produces a prediction distinguishable from general relativity at present observational precision; the deviations are confined to scales `r ≲ ℓ0 ≲ 0.1 r_s`. This is stated plainly rather than dressed up: consistency with EHT, LIGO, and NICER is evidence the model isn't already ruled out, not evidence that it's uniquely correct.
