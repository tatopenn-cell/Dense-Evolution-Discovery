...
On this view, we are ourselves pressure points in the cosmic vacuum. Atomic nuclei being vanishingly small relative to the atom is read here as physical evidence that matter is not solid stuff but a vibrating dynamic pressure &mdash; the same pressure-point mechanism at a different scale. A pressure point can always disappear, and the universe's way of not repeating a disappearance is active, not passive: unstable configurations that fail are stored as dark matter, encoded as gravitational memory, so that the same failure is not repeated as easily the next time. Persistence, in this picture, is closer to error avoidance than to a static tendency to survive.
