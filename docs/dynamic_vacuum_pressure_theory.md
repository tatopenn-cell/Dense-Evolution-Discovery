...
a symmetric double-well potential whose two minima at `u = ±1` represent the vacuum's two stable pressure states, whose maximum at `u = 0` represents the instability threshold between them. Gradient descent on this functional gives the reaction-diffusion equation `∂u/∂t = d²u/dr² - u + 2u³`, whose stationary solutions satisfy `d²u/dr² = u - 2u³`.
...
A separate, refined test restricts to four initial conditions (dropping the two-step case) and studies grid convergence directly: correlation with `sech(r)` rises from 0.909 at N=200 to 0.995 at N=2000 and stabilizes there &mdash; a real, reproducible convergence, but to **0.995**, with a residual deviation from `sech` of up to 0.06 near `r ≈ 0.7-1ℓ0` that does not vanish even at the finest grid tested. As a discriminating control, an asymmetric variant of the potential (`V₂(u) = u²(1-u²)`, lacking the symmetric double well) produces no attractor at all &mdash; the dynamics diverges (correlation -0.145, amplitude unbounded) &mdash; confirming the symmetric double-well structure specifically, not just "some" nonlinear potential, is what selects `sech`-like behavior for the initial conditions that do converge.
...
**What `P[u]` actually is, named plainly.** `P[u]` above is a standard Ginzburg-Landau free-energy functional &mdash; the same mathematical object used for a superconductor's order parameter or any second-order phase transition in the &phi;<sup>4</sup> universality class. Nothing new is being invented here: the double-well potential and its relaxational (gradient-flow) dynamics are textbook statistical field theory, and identifying the vacuum's regularization profile with this specific, well-studied object is itself a real, checkable claim &mdash; verified symbolically: `u(r)=sech(r/ℓ0)` is exactly the fixed point of `∂u/∂t = -δP/δu`, with zero residual.

Two honest limits of this result, checked directly rather than assumed. First, the same free-energy mechanism does **not** extend to the `n=2` exponent actually used in Sections 3 and 6 (only Level 3's area-law argument fixes that exponent) &mdash; a direct symbolic check shows `sech((r/ℓ0)²)`'s second derivative has explicit, non-removable `r`-dependence, so no `r`-translation-invariant free energy of this type can produce it. Second, and more fundamentally: this `P[u]` is a *dissipative* relaxation, not a Lagrangian field theory with a stress-energy tensor, so asking whether "this field" gravitationally sources the Hayward-`cosh` metric used elsewhere in this paper is checking a different, stronger claim. Promoting `u` to a genuine minimally-coupled canonical scalar `φ` and checking this directly gives a clean **no**: any metric of the `ds²=-f dt²+dr²/f+r²dΩ²` form used throughout this paper has `G^t_t≡G^r_r`, forcing `ρ=-p_r` by Einstein's equations, while a canonical scalar always has `ρ+p_r=f·φ'²>0` wherever its profile is non-constant &mdash; a direct contradiction unless `φ` is trivial. This is not a special failure of this theory; it is the same structural reason the regular-black-hole literature sources these metrics with nonlinear electrodynamics or explicit anisotropic fluids rather than ordinary scalar fields [26]. The Level 5 mechanism explains the regularization's *shape*; it is not, and cannot also be, what gravitationally produces it &mdash; these remain two separate, unconnected pieces of the picture.
...
<tr><td style="padding:8px 10px;border-bottom:1px solid #eef0f2">This scalar field gravitationally sources the metric</td><td style="padding:8px 10px;border-bottom:1px solid #eef0f2">Refuted</td><td style="padding:8px 10px;border-bottom:1px solid #eef0f2">5</td><td style="padding:8px 10px;border-bottom:1px solid #eef0f2">Proven impossible: needs <code>ρ=-p_r</code>, a canonical scalar gives <code>ρ+p_r=fφ'²&gt;0</code></td></tr>
