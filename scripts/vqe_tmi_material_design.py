...
lowest-energy result per U -- a standard defense against landing in a bad
local minimum, not a cosmetic detail (see the expressivity gap reported
below).
...
The RY-CX-RZ ansatz used here reaches the exact ground state at U=0 but
its gap to the true ground energy grows with U (roughly 0.02 eV at
U=0.55 up to ~0.5 eV at U=6.0) -- an expressivity limitation of this
fixed circuit in the strongly-correlated regime, not an optimizer failure
(multi-start restarts consistently converge to the same plateau at high
U).
...
confirmed PBE/STO-3G calculation at the Ga-As nearest-neighbor bond
length (2.44 A, the zinc-blende distance). U is that same calculation's
on-site Coulomb integral, converted from its gas-phase value to an
approximate solid-state value by dividing by GaAs's static dielectric
constant (epsilon_r = 12.9, a textbook-cited value, e.g. Sze) -- the
standard way to account for the screening a solid's surrounding crystal
provides that an isolated gas-phase pair cannot. GaAs is not
conventionally modeled as a Hubbard material, so there is no
...
19). t2/phi (the Haldane-phase term) have no independent GaAs source at
all -- no measured "topological phase" for this material -- and are kept
at the same T2/T1 ratio as the toy sweep, an explicit illustrative
choice, not a physical parameter.

Produces `data/vqe_tmi_material_design.csv`,
`data/vqe_tmi_material_design_gaas.csv`, and
...
# GaAs point (see dft_gaas_valence_parameters.py): converged,
# wavefunction-stability-confirmed PBE/STO-3G SCF at the Ga-As
...
# divides it by GaAs's static dielectric constant (Sze) to approximate
# the solid-state value. T2_GAAS_EV has no independent GaAs source and
# is kept at the toy sweep's T2/T1 ratio.
...
def build_tmi_hamiltonian(u_val, n_qubits=N_QUBITS, t1=T1, t2=T2, phi=PHI):
    ...
    B = qubits[n/2:]) + nearest-neighbor hopping t1 + complex
...
def _adam_optimize(h_list, n_starts=N_STARTS, n_epochs=N_EPOCHS, lr=LEARNING_RATE, seed=0):
    """Core batched Adam loop shared by the U sweep and the GaAs
...
def run_real_gaas_point(n_starts=N_STARTS, n_epochs=N_EPOCHS, lr=LEARNING_RATE, seed=1):
    ...
    the GaAs point: t1 = T1_GAAS_DFT_EV (Ga-As bond length, DFT)
    and U = U_GAAS_SCREENED_EV (the DFT on-site integral, dielectrically
    screened by GaAs's static dielectric constant)."""
...
if __name__ == "__main__":
    ...
    print("============================================================")
    print("Topological Mott Isolator: VQE optimization vs. exact diagonalization")
    print("============================================================")
    ...
    print("\n============================================================")
    print("GaAs point: DFT-derived hopping, dielectrically-screened U")
    print("============================================================")
    print(f"t1 (DFT, Ga-As bond 2.44 A): {T1_GAAS_DFT_EV:.4f} eV")
    ...
    assert np.all(gaas_gap > -1e-6), (
        f"Variational principle violated at the GaAs point: gap={gaas_gap.min():.6f}"
    )
    ...
    ax2.bar(x - width / 2, [e_exact_gaas], width, color="#2ecc71", label="Exact ground state")
    ax2.bar(x + width / 2, [e_vqe_gaas], width, color="#3498db", label="VQE-optimized")
    pad = max(abs(e_vqe_gaas - e_exact_gaas) * 2, 0.05)
    ax2.set_ylim(min(e_exact_gaas, e_vqe_gaas) - pad, max(e_exact_gaas, e_vqe_gaas) + pad)
    ax2.set_xticks(x)
